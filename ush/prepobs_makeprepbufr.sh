...
#            NPROCS=$(echo $PBS_HOSTS|wc -w)
            NPROCS=4 #3 IG NOT SURE what's going on here ??? IG
...
#            mpirun.lsf cfp $DATA/prep_exec.cmd
            mpiexec -np 6 --cpu-bind verbose,core cfp $DATA/prep_exec.cmd # ?? IG
            export err=$?; $DATA/err_chk
            [ $err != 0 ] && exit 55  # for extra measure
...
#            $launcher_PREP -n 1 $DATA/prepthrds.sh
         mpiexec -np 6 --cpu-bind verbose,core cfp $DATA/prepthrds.sh # 6 so it's > NSPLIT=4 in job card
	 else
            $DATA/prepthrds.sh
         fi
      fi
