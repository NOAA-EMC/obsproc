...
# save snapshot of prepbufr file after PREPOBS_PREPDATA in COMOUT
   cpfs prepda.prepdata ${COMOUT}/${RUN}.${cycle}.prepbufr_pre-qc.tm00
   ...
   if [ "$CHGRP_RSTPROD" = 'YES' ]; then
      ...
      if [ $errch -eq 0 ]; then
         ...
         cpfs /dev/null ${COMOUT}/${RUN}.${cycle}.prepbufr_pre-qc.tm00
         warning=yes
      fi
   fi
...
#  into PREPBUFR file and used by q.c. programs) in COMOUT
   if [ -s sgesprepA ]; then
      cpfs sgesprep  $COMOUT/${RUN}.${cycle}.sgesprep_before.tm00
      cpfs sgesprepA $COMOUT/${RUN}.${cycle}.sgesprep_after.tm00
   else
      cpfs sgesprep $COMOUT/${RUN}.${cycle}.sgesprep.tm00
   fi
...
# save path name of this global sigma guess file in COMOUT
   if [ -s sgesprepA_pathname ]; then
      ...
      cpfs sgesprep_pathname $COMOUT/${RUN}.${cycle}.sgesprep_pathname.tm00
   fi
fi
...
# save final form of prepbufr file in COMOUT
   cpfs prepda.${cycle} $COMOUT/${RUN}.${cycle}.prepbufr.tm00
   if [ "$CHGRP_RSTPROD" = 'YES' ]; then
      ...
      if [ $errch -eq 0 ]; then
         ...
         cpfs /dev/null $COMOUT/${RUN}.${cycle}.prepbufr.tm00
         warning=yes
      fi
   fi
   if [ "$SENDDBN" = 'YES' ] ; then
   ...
   if [ -s prepbufr.acft_profiles ]; then
      ...
      if [ "$CHGRP_RSTPROD" = 'YES' ]; then
         ...
         if [ $errch -eq 0 ]; then
            ...
            cpfs /dev/null $COMOUT/${RUN}.${cycle}.prepbufr.acft_profiles.tm00
            warning=yes
         fi
      fi
   fi
...
# save prepacqc output files in COMOUT
   if [ -s acftqc_*.sus ]; then
      ...
      cpfs acftqc_sus $COMOUT/${RUN}.${cycle}.acqc_sus.tm00
      if [ "$CHGRP_RSTPROD" = 'YES' ]; then
         ...
         if [ $errch -eq 0 ]; then
            ...
            cpfs /dev/null $COMOUT/${RUN}.${cycle}.acqc_sus.tm00
            warning=yes
         fi
      fi
   fi
   ...
   if [ -s acftqc_*.stk ]; then
      ...
      cpfs acftqc_stk $COMOUT/${RUN}.${cycle}.acqc_stk.tm00
      if [ "$CHGRP_RSTPROD" = 'YES' ]; then
         ...
         if [ $errch -eq 0 ]; then
            ...
            cpfs /dev/null $COMOUT/${RUN}.${cycle}.acqc_stk.tm00
            warning=yes
         fi
      fi
   fi
   ...
   if [ -s acftqc_*.spk ]; then
      ...
      cpfs acftqc_spk $COMOUT/${RUN}.${cycle}.acqc_spk.tm00
      if [ "$CHGRP_RSTPROD" = 'YES' ]; then
         ...
         if [ $errch -eq 0 ]; then
            ...
            cpfs /dev/null $COMOUT/${RUN}.${cycle}.acqc_spk.tm00
            warning=yes
         fi
      fi
   fi
   ...
   if [ -s acftqc_*.ord ]; then
      ...
      cpfs acftqc_ord $COMOUT/${RUN}.${cycle}.acqc_ord.tm00
      if [ "$CHGRP_RSTPROD" = 'YES' ]; then
         ...
         if [ $errch -eq 0 ]; then
            ...
            cpfs /dev/null $COMOUT/${RUN}.${cycle}.acqc_ord.tm00
            warning=yes
         fi
      fi
   fi
   ...
   if [ -s acftqc_*.lst ]; then
      ...
      cpfs acftqc_lst $COMOUT/${RUN}.${cycle}.acqc_lst.tm00
      if [ "$CHGRP_RSTPROD" = 'YES' ]; then
         ...
         if [ $errch -eq 0 ]; then
            ...
            cpfs /dev/null $COMOUT/${RUN}.${cycle}.acqc_lst.tm00
            warning=yes
         fi
      fi
   fi
   ...
   if [ -s acftqc_*.inv ]; then
      ...
      cpfs acftqc_inv $COMOUT/${RUN}.${cycle}.acqc_inv.tm00
      if [ "$CHGRP_RSTPROD" = 'YES' ]; then
         ...
         if [ $errch -eq 0 ]; then
            ...
            cpfs /dev/null $COMOUT/${RUN}.${cycle}.acqc_inv.tm00
            warning=yes
         fi
      fi
   fi
   ...
   if [ -s acftqc_*.inc ]; then
      ...
      cpfs acftqc_inc $COMOUT/${RUN}.${cycle}.acqc_inc.tm00
      if [ "$CHGRP_RSTPROD" = 'YES' ]; then
         ...
         if [ $errch -eq 0 ]; then
            ...
            cpfs /dev/null $COMOUT/${RUN}.${cycle}.acqc_inc.tm00
            warning=yes
         fi
      fi
   fi
   ...
   if [ -s acftqc_*.grc ]; then
      ...
      cpfs acftqc_grc $COMOUT/${RUN}.${cycle}.acqc_grc.tm00
      if [ "$CHGRP_RSTPROD" = 'YES' ]; then
         ...
         if [ $errch -eq 0 ]; then
            ...
            cpfs /dev/null $COMOUT/${RUN}.${cycle}.acqc_grc.tm00
            warning=yes
         fi
      fi
   fi
   ...
   if [ -s acftqc_*.dup ]; then
      ...
      cpfs acftqc_dup $COMOUT/${RUN}.${cycle}.acqc_dup.tm00
      if [ "$CHGRP_RSTPROD" = 'YES' ]; then
         ...
         if [ $errch -eq 0 ]; then
            ...
            cpfs /dev/null $COMOUT/${RUN}.${cycle}.acqc_dup.tm00
            warning=yes
         fi
      fi
   fi
   ...
   if [ -s acftqc_*.log ]; then
      ...
      cpfs acftqc_log $COMOUT/${RUN}.${cycle}.acqc_log.tm00
      if [ "$CHGRP_RSTPROD" = 'YES' ]; then
         ...
         if [ $errch -eq 0 ]; then
            ...
            cpfs /dev/null $COMOUT/${RUN}.${cycle}.acqc_log.tm00
            warning=yes
         fi
      fi
   fi
   ...
   if [ -s merged.reports.post_acftobs_qc.sorted ]; then
      ...
      if [ "$CHGRP_RSTPROD" = 'YES' ]; then
         ...
         if [ $errch -eq 0 ]; then
            ...
            cpfs /dev/null $COMOUT/${RUN}.${cycle}.acqc_merged_sorted.tm00
            warning=yes
         fi
      fi
   fi
   ...
   if [ -s merged.profile_reports.post_acftobs_qc.sorted ]; then
      ...
      if [ "$CHGRP_RSTPROD" = 'YES' ]; then
         ...
         if [ $errch -eq 0 ]; then
            ...
            cpfs /dev/null $COMOUT/${RUN}.${cycle}.acqc_merged.prof_sorted.tm00
            warning=yes
         fi
      fi
   fi
...
# save cqcbufr output files in COMOUT
   touch cqc_events
   cpfs cqc_events $COMOUT/${RUN}.${cycle}.cqc_events.tm00
   touch cqc_stncnt
   cpfs cqc_stncnt $COMOUT/${RUN}.${cycle}.cqc_stncnt.tm00
   touch cqc_stnlst
   cpfs cqc_stnlst $COMOUT/${RUN}.${cycle}.cqc_stnlst.tm00
   touch cqc_sdm
   cpfs cqc_sdm $COMOUT/${RUN}.${cycle}.cqc_sdm.tm00
   touch cqc_radcor
   cpfs cqc_radcor $COMOUT/${RUN}.${cycle}.cqc_radcor.tm00

# save cqcvad bird algorithm q.c. info list in COMOUT
   touch cqcvad.birdqc
   cpfs cqcvad.birdqc  $COMOUT/${RUN}.${cycle}.cqcvad.birdqc.tm00
