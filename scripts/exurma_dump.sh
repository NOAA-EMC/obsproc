...
echo "         Apr 29 2022 - re-added 5th dump, HR-Metar nc255033/  "
echo "                       metarh                                 "
echo "         Apr 29 2022 - re-added 6th dump: vadwnd & adpupa     "
echo "         Apr 29 2022 - re-added 7th dump: proflr & rassda     "
echo "         Apr 29 2022 - re-added 8th dump: gpsipw              "
echo "         Apr 29 2022 - re-added 9th dump: aircft, aircar      "
echo "         Apr 29 2022 - re-added 10th dump: various brightness "
echo "                       temperatures and radiances             "
echo "         Apr 29 2022 - re-added 11th dump: nexrad             "
echo "         Apr 29 2022 - re-added 12th dump: various other      "
echo "                       brightness temperatures and radiances  "
echo "         Apr 29 2022 - added urma_ru component                "
echo "         Aug  1 2022 - Added SUBPFL, SALDRN, SNOCVR,          "
echo "                       and GMI1CR types.                      "
echo "         Dec  7 2022 - re-added 5th dump, HR-Metar nc255033/  "
echo "                       metarh                                 "
echo "                     - re-added 6th dump: vadwnd & adpupa     "
echo "                     - re-added 7th dump: proflr & rassda     "
echo "                     - re-added 8th dump: gpsipw              "
echo "                     - re-added 9th dump: aircft, aircar      "
echo "                     - re-added 10th dump: various brightness "
echo "                       temperatures and radiances             "
echo "                     - re-added 11th dump: nexrad             "
echo "                     - re-added 12th dump: various other      "
echo "                       brightness temperatures and radiances  "
echo "                     - re-added urma_ru component             "
echo "                     - Removed saphir data from Group12.      "
#####################################################################
...
def_time_window_6=2.5  # default time window for dump 5 is -2.5 to +2.5
...
$ushscript_dump/bufr_dump_obs.sh $dumptime ${def_time_window_6} 1 vadwnd adpupa
error6=$?
...
def_time_window_7=0.5 # default time window for dump 7 is -0.5 to +0.5 hours
...
$ushscript_dump/bufr_dump_obs.sh $dumptime ${def_time_window_7} 1 proflr rassda
error7=$?
...
#============================================================================
# Dump # 8 :  ...    GPSIPW -- TOTAL NUMBER OF SUBTYPES = 1
#                      (1)
#============================================================================

def_time_window_8=0.5 # default time window for dump 8 is -0.5 to +0.5 hours
...
$ushscript_dump/bufr_dump_obs.sh $dumptime ${def_time_window_8} 1 gpsipw
error8=$?
...
#===========================================================================
# Dump # 9 : AIRCFT, AIRCAR -- TOTAL NUMBER OF SUBTYPES = 10
#              (8)     (2)
#===========================================================================
export LALO=0  # GLOBAL dumps here (AIRCFT and AIRCAR dumped globally to
               # improve PREPOBS_PREPACQC track-check performance)

def_time_window_9=3.25 # default time window for dump 9 is -3.25 to +3.25 hours
...
$ushscript_dump/bufr_dump_obs.sh $dumptime ${def_time_window_9} 1 aircft aircar
error9=$?
...
#===============================================================================
# Dump # 10 : 1BAMUA, 1BMHS,  ESAMUA, ESMHS, ATMS, MTIASI, SEVCSR, GPSRO,
#              (1)    (1)      (1)     (1)   (1)    (1)    (1)     (1)
...
def_time_window_10=3.0 # default time window for dump 10 is -3.0 to +3.0 hours
...
$ushscript_dump/bufr_dump_obs.sh $dumptime ${def_time_window_10} 1 1bamua \
...
#===========================================================================
# Dump # 11 : NEXRAD -- TOTAL NUMBER OF SUBTYPES = 4
#              (4)
#===========================================================================
...
def_time_window_11=0.5 # default time window for dump 11 is -0.5 to +0.5 hours
...
$ushscript_dump/bufr_dump_obs.sh $dumptime ${def_time_window_11} 1 nexrad
error11=$?
...
#==========================================================================
# Dump # 12 : 1BHRS4, AIRSEV, LGHTNG, ESHRS3, LGYCLD, CRISF4, SSMISU, OSBUV8,
#               (1)     (1)     (2)     (1)     (1)    (1)     (1)     (1)
#             CRSFDB
#               (1)
...
$ushscript_dump/bufr_dump_obs.sh $dumptime ${def_time_window_12} 1 1bhrs4 \
...
if [ "$PROCESS_DUMP" = 'YES' ]; then
   ...
   if [ "$err1" -gt '5' -o "$err2" -gt '5' -o "$err3" -gt '5' -o "$err4" -gt '5' \
     -o "$err5" -gt '5' -o "$err6" -gt '5' -o "$err7" -gt '5' -o "$err8" -gt '5' \
     -o "$err9" -gt '5' -o "$err10" -gt '5' -o "$err11" -gt '5' -o "$err12" -gt '5' ]; then
...
echo
echo " ###################################################### "
echo " --> > 22 RETURN CODE FROM DATA DUMP, $err1, $err2, $err3, $err4, $err5, $err6, $err7, $err8, $err9, $err10, $err11, $err12"
...
## is non-fatal but still worth noting
      ...
      echo
      echo " ###################################################### "
      echo " --> > 5 RETURN CODE FROM DATA DUMP, $err1, $err2, $err3, $err4, $err5, $err6, $err7, $err8, $err9, $err10, $err11, $err12"
