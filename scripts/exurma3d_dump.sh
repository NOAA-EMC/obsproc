...
#####################################################################
echo "----------------------------------------------------------    "
echo "exurma3d_dump.sh - URMA model data dump processing          "
echo "----------------------------------------------------------    "
...
msg="URMA3D ANALYSIS TIME IS $PDY$cyc"
[ -n "$cycM" ]  &&  msg="$msg:${cycM}"
postmsg "$jlogfile" "$msg"
...
set +x
echo
echo "CENTER DATA DUMP DATE-TIME FOR $tmmark_uc URMA3D IS $dumptime"
echo
set -x
...
msg="START THE $tmmark_uc URMA3D DATA DUMP CENTERED ON $dumptime"
postmsg "$jlogfile" "$msg"
