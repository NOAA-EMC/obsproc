...
#####################################################################
echo "----------------------------------------------------------    "
echo "exrtma3d_dump.sh - RTMA3D model data dump processing          "
echo "----------------------------------------------------------    "
echo "History:                                                      "
...
msg="RTMA3D ANALYSIS TIME IS $PDY$cyc"
[ -n "$cycM" ]  &&  msg="$msg:${cycM}"
postmsg "$jlogfile" "$msg"

set +x
echo
echo "CENTER DATA DUMP DATE-TIME FOR $tmmark_uc RTMA3D IS $dumptime"
echo
set -x
...
msg="START THE $tmmark_uc RTMA3D DATA DUMP CENTERED ON $dumptime"
postmsg "$jlogfile" "$msg"
...
DTIM_earliest_ascatt=${DTIM_earliest_ascatt:-"-3.00"}
DTIM_latest_ascatt=${DTIM_latest_ascatt:-"+3.00"}

DTIM_earliest_efclam=${DTIM_earliest_efclam:-"-0.50"}
DTIM_latest_efclam=${DTIM_latest_efclam:-"+0.50"}
...
$ushscript_dump/bufr_dump_obs.sh $dumptime 1.5 1 ascatt efclam snocvr gmi1cr
error1=$?
...
# for rtma3d_ru_0000 read only from previous day's tank
# Temporary bug fix
if [ $RUN = "rtma3d_ru" -a $cycle = "t0000z" ]; then
...
def_time_window_7=2.5  # default time window for dump 5 is -2.5 to +2.5
                       # hours for all other cycles

# Time window is -1.00 to +1.00 hours for VADWND
#  (note: time window increased over +/- 0.5 hr standard to get more data)
i  DTIM_earliest_vadwnd=${DTIM_earliest_vadwnd:-"-1.50"}
...
$ushscript_dump/bufr_dump_obs.sh $dumptime ${def_time_window_7} 1 vadwnd adpupa
error7=$?
...
#========================================================================
# Dump # 8 : PROFLR, RASSDA, SATMAR, SOFARW
#              (3)     (1)
...
def_time_window_8=1.5 # default time window for dump 5 is -0.5 to +0.5 hours
...
$ushscript_dump/bufr_dump_obs.sh $dumptime ${def_time_window_8} 1 proflr rassda satmar sofarw
error8=$?
...
#============================================================================
# Dump # 9 :  ...    GPSIPW -- TOTAL NUMBER OF SUBTYPES = 1
#                      (1)
#============================================================================

def_time_window_9=0.5 # default time window for dump 9 is -0.5 to +0.5 hours
...
$ushscript_dump/bufr_dump_obs.sh $dumptime ${def_time_window_9} 1 gpsipw
error9=$?
...
#===========================================================================
# Dump # 10 : AIRCFT, AIRCAR, GOESND -- TOTAL NUMBER OF SUBTYPES = 12
#              (8)     (2)     (2)
#===========================================================================
...
def_time_window_10=0.75 # default time window for dump 10 is -3.25 to +3.25 hours
...
$ushscript_dump/bufr_dump_obs.sh $dumptime ${def_time_window_10} 1 aircft \
                                                                  aircar
...
#===============================================================================
# Dump # 11 : 1BAMUA, 1BMHS,  ESAMUA, ESMHS, ATMS, MTIASI, SEVCSR, GPSRO,
#              (1)    (1)      (1)     (1)   (1)    (1)    (1)     (1)
...
$ushscript_dump/bufr_dump_obs.sh $dumptime ${def_time_window_11} 1 1bamua \
...
#===========================================================================
# Dump # 12 : NEXRAD -- TOTAL NUMBER OF SUBTYPES = 4
#              (4)
#===========================================================================
...
def_time_window_12=1.5 # default time window for dump 12 is -0.5 to +0.5 hours
...
# !!! this needs adjusting for 15min cycling RTMA3D_RU -JW !!!
...
$ushscript_dump/bufr_dump_obs.sh $dumptime ${def_time_window_12} 1 nexrad
error12=$?
...
#==========================================================================
# Dump # 13 : 1BHRS4, AIRSEV, LGHTNG, ESHRS3, LGYCLD, CRISF4, SSMISU, OSBUV8,
#               (1)     (1)     (2)     (1)     (1)    (1)     (1)     (1)
#             CRSFDB
#               (1) 
...
def_time_window_13=3.0 # default time window for dump 13 is -3.0 to +3.0 hours
...
$ushscript_dump/bufr_dump_obs.sh $dumptime ${def_time_window_13} 1 1bhrs4 \
...
#==========================================================================
# Dump # 15 :  ADPUPA minus UPRAIR
#               (6)   minus  (5)
#            -- TOTAL NUMBER OF SUBTYPES = 6-5
#==========================================================================
#

def_time_window_15=1.0 # default time window for dump 15 is -1.0 to +1.0 hours

DTIM_earliest_uprair=${DTIM_earliest_uprair:-"-1.00"}
DTIM_latest_uprair=${DTIM_latest_uprair:-"+1.00"}
                                                                                    
$ushscript_dump/bufr_dump_obs.sh $dumptime ${def_time_window_15} 1 uprair
error15=$?
...
if [ "$PROCESS_DUMP" = 'YES' ]; then
   ...
   if [ "$err1" -gt '5' -o "$err2" -gt '5' -o "$err3" -gt '5' \
     ...
     -o "$err13" -gt '5' -o "$err14" -gt '5' -o "$err15" -gt '5' ] ; then
...
if [ "$RUN" == "rtma3d_ru" ] && [ "${SENDDBN^^}" = YES ] && [ -s ${COMSP}satwnd.tm00.bufr_d ]; then
   $DBNROOT/bin/dbn_alert MODEL RTMA3D_RU_BUFR_satwnd $job ${COMSP}satwnd.tm00.bufr_d
fi
